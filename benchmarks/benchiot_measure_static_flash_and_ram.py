...
def write_results_to_json_file(flash_size, ram_size, exec_code_size, rop_gadgets, max_code_ratio, max_data_ratio, size_file):
    ...
    bench_json_result_file = "generated/benchmarks/"+ bench_name + "/" + bench_name + "_benchiot" + JSON_EXT
    ...
    # update json results
    json_results["Total_FLASH"] = str(flash_size)
    json_results["Total_RAM"] = str(ram_size) #+ stack_heap
    json_results["Executable_code"] = str(exec_code_size)
    json_results["Max_code_ratio"] = str(max_code_ratio)
    json_results["Max_data_ratio"] = str(max_data_ratio)
    json_results["ROP_gadgets"] = str(rop_gadgets)
    json_results["Indirect_calls"] = str(num_indrect_calls)
...
def dump_analysis_files(file_list, analysis_path, analysis_cmd, file_ext):
    ...
    try:
        ...
        for bin_file in file_list:
            analysis_filename = analysis_path + get_filename(bin_file) + "/" + get_filename(bin_file) + file_ext
...
def measure_static():
    ...
    for bench in benchmarks:
        gen_filelist(bins_path + bench, bin_filelist, BIN_EXT)
    ...
    size_filelist = []
    for bench in benchmarks:
        gen_filelist(analysis_path + bench, size_filelist, SIZE_EXT)
    # get a list of ROPresiliency files
    rop_filelist = []
    for bench in benchmarks:
        gen_filelist(analysis_path + bench, rop_filelist, ROP_EXT)
